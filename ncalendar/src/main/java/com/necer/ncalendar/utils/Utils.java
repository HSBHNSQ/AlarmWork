...
public class Utils {
     ...
     */
    public static int getIntervalDays(DateTime dateTime1,DateTime dateTime2)
    {
     ...
     * @return
     */
    public static NCalendar getMonthCalendar2(DateTime dateTime, int type,Context context) {
        ...
        List<String> lunarList = new ArrayList<>();
        List<String> shiftWorkList = new ArrayList<>();
        DateTime startDataTime = DataUtil.getInstance().getShiftWorkStartTime(context);
        List<String> shiftWorkCycle = DataUtil.getInstance().getShiftWorkCycleList(context);
        if (shiftWorkCycle.size() == 0){
            shiftWorkCycle.add("");
        }
        //周日开始的
        if (type == 0) {
            ...
            if (firstDayOfWeek != 7) {
                for (int i = 0; i < firstDayOfWeek; i++) {
                    ...
                    String lunarDayString = LunarCalendarUtils.getLunarDayString(dateTime1.getYear(), dateTime1.getMonthOfYear(), dateTime1.getDayOfMonth(), lunar.lunarYear, lunar.lunarMonth, lunar.lunarDay, lunar.isLeap);
                    lunarList.add(lunarDayString);
                    if (startDataTime != null && shiftWorkCycle != null){
                        int dt = Utils.getIntervalDays(startDataTime,dateTime1);
                        int index =dt % shiftWorkCycle.size();;
                        if (index < 0){
                            index = index + shiftWorkCycle.size();
                        }
                        shiftWorkList.add(shiftWorkCycle.get(index));
                    }
                }
            }
            //当月
        ...
        nCalendar.lunarList = lunarList;
        nCalendar.shiftWorkList = shiftWorkList;
        return nCalendar;
    }
    ...
    public static NCalendar getWeekCalendar2(DateTime dateTime, int type,Context context) {
        List<DateTime> dateTimeList = new ArrayList<>();
        ...
        calendar.shiftWorkList = shiftWorkList;
        return calendar;
    }
    ...
    public static class NCalendar {
        ...
        public List<String> shiftWorkList;
    }
