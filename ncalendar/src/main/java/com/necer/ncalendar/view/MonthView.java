...
public class MonthView extends CalendarView {
    ...
    public MonthView(Context context,
        ...
        initDate(context,dateTime);

    }
    private void initDate(Context context,DateTime dateTime){
        Utils.NCalendar nCalendar2 = Utils.getMonthCalendar2(dateTime, Attrs.firstDayOfWeek,context);
        lunarList = nCalendar2.lunarList;
        dateTimes = nCalendar2.dateTimeList;
        shiftWorkList = nCalendar2.shiftWorkList;
        mRowNum = dateTimes.size() / 7;
    }

    @Override
    public void refresh(){
        initDate(getContext(),mInitialDateTime);
        invalidate();
    }
        ...
        for (int i = 0; i < mRowNum; i++) {
            for (int j = 0; j < 7; j++) {
                ...
                if (mRowNum == 5) {
                    ...
                    baseline = (rect.bottom + rect.top - fontMetrics.bottom - fontMetrics.top) / 2
                                + (mHeight / 5 - mHeight / 6) / 4 - rect.height() / 4;
                }
...
//                        canvas.drawCircle(rect.centerX(), centerY, mSelectCircleRadius, mSorlarPaint);
                        ...
                        drawLunar(canvas, rect, baseline, Color.WHITE, i, j);
                        //绘制节假日
            ...
            for (int i = 0; i < mRectList.size(); i++) {
                ...
                if (rect.contains((int) e.getX(), (int) e.getY())) {
                    ...
                    if (Utils.isLastMonth(selectDateTime, mInitialDateTime)) {
                        mOnClickMonthViewListener.onClickLastMonth(selectDateTime,shiftWork);
                    } else if (Utils.isNextMonth(selectDateTime, mInitialDateTime)) {
                        mOnClickMonthViewListener.onClickNextMonth(selectDateTime,shiftWork);
                    } else {
                        mOnClickMonthViewListener.onClickCurrentMonth(selectDateTime,shiftWork);
                    }
                    break;
                }
            }
